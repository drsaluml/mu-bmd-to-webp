...
// RasterizeTriangleAdditiveOverlay is like RasterizeTriangleAdditive but ONLY
// renders on pixels where opaque geometry already exists (zbuf != -inf).
// This prevents dark overlay panels from creating a "black aura" on empty background.
func RasterizeTriangleAdditiveOverlay(
	fb *FrameBuffer,
	px, py, pz []float64,
	uvs [][2]float32,
	vi [3]int, ti [3]int,
	...
	for sy := minY; sy <= maxY; sy++ {
		...
		for sx := minX; sx <= maxX; sx++ {
			...
			// OVERLAY CHECK: skip pixels on empty background (no opaque geometry drawn)
			if fb.ZBuf[zbIdx] == math.Inf(-1) {
				continue
			}
