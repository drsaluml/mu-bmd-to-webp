...
// marsCipher implements MARS block cipher decryption.
// 16-byte block, 16-byte key.
type marsCipher struct {
	lKey [40]uint32
}

func newMARS(key []byte) blockCipher {
	c := &marsCipher{}
	c.setKey(key[:16])
	return c
}

func (c *marsCipher) BlockSize() int { return 16 }

func (c *marsCipher) DecryptBlock(src, dst []byte) {
	K := c.lKey

	d := binary.LittleEndian.Uint32(src[0:]) + K[36]
	cc := binary.LittleEndian.Uint32(src[4:]) + K[37]
	b := binary.LittleEndian.Uint32(src[8:]) + K[38]
	a := binary.LittleEndian.Uint32(src[12:]) + K[39]

	// Forward mixing
	a, b, cc, d = marsFMix(a, b, cc, d); a += d
	b, cc, d, a = marsFMix(b, cc, d, a); b += cc
	cc, d, a, b = marsFMix(cc, d, a, b)
	d, a, b, cc = marsFMix(d, a, b, cc)
	a, b, cc, d = marsFMix(a, b, cc, d); a += d
	b, cc, d, a = marsFMix(b, cc, d, a); b += cc
	cc, d, a, b = marsFMix(cc, d, a, b)
	d, a, b, cc = marsFMix(d, a, b, cc)
	...
	a, b, cc, d = marsRKtr(a, b, cc, d, K[:], 34)
	b, cc, d, a = marsRKtr(b, cc, d, a, K[:], 32)
	cc, d, a, b = marsRKtr(cc, d, a, b, K[:], 30)
	d, a, b, cc = marsRKtr(d, a, b, cc, K[:], 28)
	a, b, cc, d = marsRKtr(a, b, cc, d, K[:], 26)
	b, cc, d, a = marsRKtr(b, cc, d, a, K[:], 24)
	cc, d, a, b = marsRKtr(cc, d, a, b, K[:], 22)
	d, a, b, cc = marsRKtr(d, a, b, cc, K[:], 20)
	a, d, cc, b = marsRKtr(a, d, cc, b, K[:], 18)
	b, a, d, cc = marsRKtr(b, a, d, cc, K[:], 16)
	cc, b, a, d = marsRKtr(cc, b, a, d, K[:], 14)
	d, cc, b, a = marsRKtr(d, cc, b, a, K[:], 12)
	a, d, cc, b = marsRKtr(a, d, cc, b, K[:], 10)
	b, a, d, cc = marsRKtr(b, a, d, cc, K[:], 8)
	cc, b, a, d = marsRKtr(cc, b, a, d, K[:], 6)
	d, cc, b, a = marsRKtr(d, cc, b, a, K[:], 4)

	// Backward mixing
	a, b, cc, d = marsBMix(a, b, cc, d)
	b, cc, d, a = marsBMix(b, cc, d, a); cc -= b
	cc, d, a, b = marsBMix(cc, d, a, b); d -= a
	d, a, b, cc = marsBMix(d, a, b, cc)
	a, b, cc, d = marsBMix(a, b, cc, d)
	b, cc, d, a = marsBMix(b, cc, d, a); cc -= b
	cc, d, a, b = marsBMix(cc, d, a, b); d -= a
	d, a, b, cc = marsBMix(d, a, b, cc)

	binary.LittleEndian.PutUint32(dst[0:], d-K[0])
	binary.LittleEndian.PutUint32(dst[4:], cc-K[1])
	binary.LittleEndian.PutUint32(dst[8:], b-K[2])
	binary.LittleEndian.PutUint32(dst[12:], a-K[3])
}

func (c *marsCipher) setKey(inKey []byte) {
	var T [15]uint32
	nk := len(inKey) / 4
	for i := 0; i < nk; i++ {
		T[i] = binary.LittleEndian.Uint32(inKey[i*4:])
	}
	T[nk] = uint32(nk)
	...
	for j := 0; j < 4; j++ {
		...
		for k := 0; k < 4; k++ {
			for i := 0; i < 15; i++ {
				T[i] = bits.RotateLeft32(T[i]+marsSBox[T[(i+14)%15]%512], 9)
			}
		}
		for i := 0; i < 10; i++ {
			c.lKey[10*j+i] = T[(4*i)%15]
		}
	}
	...
	for i := 5; i < 37; i += 2 {
		...
		m := marsGenMask(w)
		if m != 0 {
			w ^= bits.RotateLeft32(marsSBox[265+(c.lKey[i]&3)], int(c.lKey[i-1]&31)) & m
		}
		c.lKey[i] = w
	}
}

func marsGenMask(x uint32) uint32 {
	m := (^x ^ (x >> 1)) & 0x7FFFFFFF
	m &= (m >> 1) & (m >> 2)
	m &= (m >> 3) & (m >> 6)
	if m == 0 {
...
func marsFMix(a, b, c, d uint32) (uint32, uint32, uint32, uint32) {
	r := bits.RotateLeft32(a, -8)
	b ^= marsSBox[a&255]
	b += marsSBox[(r&255)+256]
	r = bits.RotateLeft32(a, -16)
	a = bits.RotateLeft32(a, -24)
	c += marsSBox[r&255]
	d ^= marsSBox[(a&255)+256]
	return a, b, c, d
}

func marsBMix(a, b, c, d uint32) (uint32, uint32, uint32, uint32) {
	r := bits.RotateLeft32(a, 8)
	b ^= marsSBox[(a&255)+256]
	c -= marsSBox[r&255]
	r = bits.RotateLeft32(a, 16)
	a = bits.RotateLeft32(a, 24)
	d -= marsSBox[(r&255)+256]
	d ^= marsSBox[a&255]
	return a, b, c, d
}

func marsRKtr(a, b, c, d uint32, K []uint32, i int) (uint32, uint32, uint32, uint32) {
	r := uint32(uint64(a) * uint64(K[i+1]) & 0xFFFFFFFF)
	a = bits.RotateLeft32(a, -13)
	m := a + K[i]
	l0 := marsSBox[m&511]
	r = bits.RotateLeft32(r, 5)
	l := l0 ^ r
	c -= bits.RotateLeft32(m, int(r&31))
	r = bits.RotateLeft32(r, 5)
	l ^= r
	d ^= r
	b -= bits.RotateLeft32(l, int(r&31))
	return a, b, c, d
}

// marsSBox is the 512-entry S-box for MARS.
var marsSBox = [512]uint32{
